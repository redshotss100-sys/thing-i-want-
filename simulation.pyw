...
class Simulation:
    ...
    def _build_world(self):
        self.world = np.zeros((GH, GW), dtype=np.uint8)
    ...
    def run(self):
        ...
        while True:
            ...
            for _ in range(self.speed):
                ...
                if now_ms - last_spawn_time >= 60000 and len(self.ants) < COLONY_SIZE and self.best_weights:
                ...
                if phero_tick >= 3:
                    ...
                    self.phero.diffuse()
                    phero_tick = 0
                early_wipe = len(self.ants) < min_alive and len(self._all_ants_this_gen) > 0 and len(self.ants) < len(self._all_ants_this_gen)
